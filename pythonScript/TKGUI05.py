...
# print(listDirs)
ttool.printInfo(os.path.dirname(cDir))
...
def getDirsByParams(childPath,flag=0):
    ...
    for path in childListDirs:
        if os.path.isfile(os.path.join(childPath, path)):
            ...
            ttool.printInfo("output dirs:",path)
        else:
            cListOnlyDirs.append(path)
            ttool.printInfo("output files:",path)
    if flag == 0:
...
def createDir():
    if not os.path.exists(jsonDir):
        os.mkdir(jsonDir)
        ttool.printInfo("Directory {} created successfully".format(jsonDir))
    else:
        ttool.printInfo("Directory {} already exists".format(jsonDir))
def getJsonFile():
    saveLists=[]
...
def createJsonFile(checkJsons):
    ...
    if len(cListFiles) == 0:
        ...
        ttool.printMsg(saveCompleteJsons)
...
def checkFiles():
    ...
    try:
        ...
        ttool.printMsg("output jsonDatas:",jsonDatas)
        ...
        for itemFile in listOnlyFiles:
            ...
            for keyName,itemJson in jsonDatas.items():
                if itemFile == itemJson:
                    ttool.printWarning("A File has been exist already")
                    recordDelDicts.append(keyName)
        ...
        ttool.printMsg("***checkJsons***:",checkJsons)
        # Not add files
        if len(checkJsons) == 0:
            checkJsons.append(-1)

    except FileNotFoundError:
        ttool.printError("File not found, please is that file path")
        createDir()
        createJsonFile(checkJsons)
        raise
    except FileReadExceptions as e:
        ttool.printError("---error:",e.msg)
        sys.exit(-1)
    except FileRUpdateExceptions as e:
        ttool.printError("---warning:",e.msg)
        raise
    return checkJsons
...
def setNewChangeFrame(newTChangeFrame):
    ttool.printInfo(newTChangeFrame)
...
def selectTree(tree,croot,cFrame):
    ...
    for item in tree.selection():
        ...
        ttool.printInfo(item_text,item)
...
def saveTextCallBack(croot,cFrame,cText,uuid):
    try:
        ...
        ttool.printError("FileSaveError")
...
def generalChangeFrame(croot,cFrame,uuid):
    ...
    insertLabelFrame = tk.LabelFrame(newRightFrame,background="#87CEFA",height=2)
    insertLabel = tk.Label(insertLabelFrame,text="Enter comments",height=2,font="Helvetica 12",background="#87CEFA")
    ...
    insertLabelFrame.pack(side="top",fill="both")
    insertLabel.pack(side="left")
...
def createGUI(croot):
    ...
    for i in listTrees:
        ...
        ttool.printMsg(tree.item(i,"values"))
...
if __name__=="__main__":
    ...
    try:
        ...
        ttool.printError("---warning:a file do not exist")
    except FileRUpdateExceptions as e:
        ttool.printWarning("---warning:",e.msg)
    else:
        createJsonFile(res)
    createGUI(getRoot())
    # '''
